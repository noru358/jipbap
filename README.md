...
이 프로젝트는 `instatoon`의 하위 포맷이나 에피소드가 아니다. `AutoPipeline` 아래에서 `instatoon`, `talkshow`와 나란히 두는 독립 child project다. 기존 프로젝트에서 가져오는 것은 검증된 상위 제작 공정 원리뿐이며, 소재 규칙·스토리 문법·말투·시각 규칙·에피소드 체계는 이 저장소가 독립적으로 소유한다.
...
- 콘티: 승인
- 말투 구조: 승인
- 원본 시각 레퍼런스: 사용자 제공 이미지; integrity/status는 `assets/REFERENCE_MANIFEST.md`
